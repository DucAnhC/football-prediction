...
interface PredictionHighlightCardProps {
  viewModel: PredictionHighlightViewModel;
}

export function PredictionHighlightCard({ viewModel }: PredictionHighlightCardProps) {
  return (
    <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/70">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="rounded-2xl bg-slate-950 px-4 py-3 text-right text-white">
          <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Mức tin cậy</p>
          <p className="mt-1 text-sm font-semibold uppercase">{viewModel.confidenceText}</p>
          <p className="mt-1 text-2xl font-semibold">{viewModel.confidenceLabel}</p>
        </div>
      </div>
