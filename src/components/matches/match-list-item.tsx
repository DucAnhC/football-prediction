...
import type { BadgeTone, MatchListCardViewModel } from "@/lib/view-models";

interface MatchListItemProps {
  viewModel: MatchListCardViewModel;
}

export function MatchListItem({ viewModel }: MatchListItemProps) {
  return (
    <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/70">
      ...
      <div className="mt-6 grid grid-cols-[1fr_auto_1fr] items-center gap-3">
        <TeamSummary team={viewModel.homeTeam} align="right" />
        <div className="min-w-[96px] rounded-3xl bg-slate-950 px-4 py-4 text-center text-white">
          <p className="text-3xl font-semibold tracking-tight">{viewModel.scoreLabel}</p>
          <p className="mt-1 text-xs uppercase tracking-[0.25em] text-slate-400">
            {viewModel.clockLabel}
          </p>
        </div>
        <TeamSummary team={viewModel.awayTeam} align="left" />
      </div>
      ...
      <div className="mt-6 flex flex-col gap-3 border-t border-slate-200 pt-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="text-sm text-slate-600">
          {viewModel.venueField ? (
            <>
              <p className="font-medium text-slate-900">{viewModel.venueField.value}</p>
              {viewModel.venueField.note ? <p className="mt-1">{viewModel.venueField.note}</p> : null}
            </>
          ) : viewModel.contextNote ? (
            <p>{viewModel.contextNote}</p>
          ) : (
            <p className="text-slate-500">Trang danh sách chỉ giữ lại dữ liệu cốt lõi cho trận này.</p>
          )}
        </div>
        <Link
          href={viewModel.href}
...
interface TeamSummaryProps {
  team: MatchListCardViewModel["homeTeam"];
  align: "left" | "right";
}

function TeamSummary({ team, align }: TeamSummaryProps) {
...
function Badge({
  ...
  badge: MatchListCardViewModel["statusBadge"];
  subtle?: boolean;
}) {
  return (
